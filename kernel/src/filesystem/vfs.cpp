...
static MountPoint *get_mount_point(const char *path)
{
...
void mount(limine_file *archive, const char *target, const char *)
{
...
File *open(const char *path)
{
    ...
    const ustar::File file
        = ustar::lookup(static_cast<const u8 *>(mount_point->archive->address), path + strlen(mount_point->target));
