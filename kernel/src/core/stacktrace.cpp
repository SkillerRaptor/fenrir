...
void initialize()
{
    ...
    logger::debug(
        "Stacktrace: Kernel symbol map found at {:#016x} with {} bytes\n",
...
void print(const u64 max_frames, const u64 rip_argument)
{
    ...
    for (usize i = 0; stack_frame && i < max_frames; ++i) {
        ...
        if (offset == 0) {
            logger::fatal(
                "  {:02u}. \033[38;2;0;0;255m{:#016x} \033[0min \033[38;2;255;215;0m{}\n",
                i + 1,
                rip,
                symbol->name);
        } else {
            logger::fatal(
                "  {:02u}. \033[38;2;0;0;255m{:#016x} \033[0min \033[38;2;255;215;0m{} \033[38;2;0;128;0m+0x{:x}\n",
                i + 1,
                rip,
                symbol->name,
                offset);
